...
- **배포 결과(자기 검수, 실제 재현)**:
  ...
  - `.env.production` 의 `DB_PASSWORD`/`APP_ENCRYPTION_KEY` 는 서버에서 `openssl rand` 로
    ...
    `APP_ADMIN_EMAILS` 는 조민석 님 실제 이메일(관리자 개인 이메일, 비공개 처리)로 설정.
...
- **실제 유저 상태**: 조민석 님이 실제 이메일(관리자 개인 이메일, 비공개 처리)로 직접 회원가입해
...
## D12. 대시보드 적자/흑자 시각화 — 차트 라이브러리 없이 SVG 도넛 + 가로 바 차트 직접 구현

- **날짜**: 2026-07-07
- **관련**: `frontend/src/components/ProfitCharts.jsx`(신규), `frontend/src/pages/Dashboard.jsx`,
  `frontend/src/styles.css`
- **배경**: 조민석 님 피드백 — "대시보드에 적자/흑자 부분을 그래프로 좀 더 직관적으로 보고
  싶다. 극적인 효과가 있어야 셀러들이 느끼는 게 크지 않을까? 지금은 너무 밋밋하다." 기존
  `ProfitTable` 은 숫자 표 형태라 적자 비중을 한눈에 느끼기 어렵다는 지적.
- **결정 — 차트 라이브러리(recharts 등)를 새로 추가하지 않고 순수 SVG+CSS 로 직접 구현**:
  `frontend/package.json` 에 차트 라이브러리가 전혀 없고, 이미 `ReturnsTable` 이
  `.bar-wrap`/`.bar`(라이브러리 없이 width% 로 그리는 가로 바)를 쓰고 있어 같은 접근을
  그대로 이었다. 이유:
  1. 운영 서버가 iwinv KR1-Lite(RAM 1GB+스왑 2GB) 라 프론트 번들 크기를 늘리는 의존성 추가는
     피하는 게 맞다.
  2. 지금 필요한 시각화(도넛 1개, 가로 바 목록 1개)는 SVG `<circle>` 의 `strokeDasharray`
     조작과 CSS width% 만으로 충분히 표현 가능해 라이브러리가 주는 이점이 크지 않다.
- **구현 내용**:
  1. `ProfitDonut` — 적자 비중을 `loss / (gain + loss)` 로 계산(상품 개수 비율이 아니라
     금액 비율 — "적자 상품이 흑자의 N% 를 갚아먹고 있다"는 문장과 직접 대응해야 의미가
     있어서). 도넛 중앙에 퍼센트 숫자, 하단에 "적자 상품이 흑자의 N%를 갚아먹고 있어요"
     같은 문장형 콜아웃을 넣어 숫자보다 문장이 먼저 읽히게 했다(요청의 "극적인 효과").
     적자 비중이 0%(적자 상품 없음)이면 긍정 문구로 분기.
  2. `ProfitBarChart` — 상품을 `profit` 절댓값 내림차순으로 정렬(흑자/적자 섞어서 "가장
     극적인 것부터"), 최대 8개로 제한(SKU 많은 셀러가 스크롤 지옥에 빠지지 않도록). 적자는
     `--loss` 색, 흑자는 `--profit` 색으로 `ReturnsTable`/`ProfitTable` 과 동일한 색상
     토큰을 재사용해 화면 간 시각 언어를 통일했다.
  3. `Dashboard.jsx` 에 `ProfitTable` 바로 위, 두 컴포넌트를 `.charts-grid`(데스크톱은
     320px+1fr 2열, `@media max-width:860px` 에서 1열)로 배치.
- **검증**: `npm run build` → 백엔드 재시작 → 데모 계정 로그인 → "최근 30일" 기간(시드 데이터가
  `settled_at=2026-06-30` 이라 "이번 달" 로는 하나도 안 보임, "최근 30일" 로 전환해야 보임)
  → 도넛 24%, 바 차트 흑자상품A/적자상품B/흑자상품C 렌더링을 Chrome MCP 스크린샷으로
  직접 확인. 렌더링된 숫자(₩407,368.42 / ₩-159,736.84 / ₩102,368.42)가 D2 기록의 시드
  값 및 화면의 `ProfitTable` 표시값과 일치함을 대조했다.
- **범위 밖으로 남긴 것**: 애니메이션(숫자 카운트업 등)·기간별 추이(월별 적자 비중 변화)
  차트는 이번 요청 범위가 아니라 손대지 않았다. 필요해지면 별도 요청으로 다룬다.

## D13. 적자 상품 진단 문구 — 원가/배분비용/광고비/반품률 비율 기반 규칙 진단(가짜 지표 아님)

- **날짜**: 2026-07-07
- **관련**: `frontend/src/components/ProfitCharts.jsx`(`diagnoseLoss`, `LossInsights`),
  `frontend/src/pages/Dashboard.jsx`, `frontend/src/styles.css`
- **배경**: D12 차트 도입 직후 조민석 님 후속 요청 — "적자인 상품은 적자인 이유와 개선책 같은
  걸 안내하는 문구를 추가해줘." 도넛/바 차트는 "얼마나" 적자인지는 보여주지만 "왜" 적자인지는
  말해주지 않는다는 공백을 메우는 것.
- **결정 — AI 호출이나 별도 통계 모델 없이, 서버가 이미 계산해 내려준 4개 실수(매출/원가/
  배분비용/광고비)의 비율만으로 규칙 기반 진단 문구를 만든다**:
  1. `HANDOFF.md` 의 "가짜 지표 금지" 원칙 때문에 사용자 후기·추정 수요 같은 걸 지어낼 수는
     없지만, 이건 다르다 — 이미 `ProfitDashboardController` 가 계산한 실측치(`cogsTotal`,
     `allocatedCost`, `adSpend`, `revenue`, `returnedUnits`/`units`)의 비율일 뿐이라 지어낸
     값이 아니다. LLM 호출 같은 무거운 의존성도 필요 없다 — 이미 있는 숫자로 결정론적 규칙만
     추가하면 됐다.
  2. `revenue = cogsTotal + allocatedCost + adSpend + profit` 관계(대시보드 부제 "정산 실수령
     - 원가 - 배분된 기타비용 - 광고비 = 진짜 순이익"과 동일 공식)를 이용해, 3개 비용 중
     매출 대비 비율이 가장 큰 항목을 "가장 큰 원인"으로 지목한다. 매출이 0 이면(비용만 발생)
     별도 문구로 분기.
  3. 반품률(`returnedUnits/units >= 10%`)은 원가/비용 비율과 독립적으로 항상 같이 검사해
     추가 문구를 붙인다 — 반품이 많으면 원가 비율이 낮아도 실제로는 개선 여지가 있는 경우가
     많아서 두 원인을 배타적으로 두지 않았다.
- **구현**: `ProfitCharts.jsx` 에 `diagnoseLoss(product)` 함수(순수 함수, UI 비의존) +
  `LossInsights({products})` 컴포넌트(적자 상품만 필터링, 최대 6개) 추가.
  `Dashboard.jsx` 에 `.charts-grid` 바로 아래·`ProfitTable` 바로 위에 배치.
- **검증**: 시드 데이터(적자상품B: revenue=270,000 / cogsTotal=405,000 / returnedUnits=5,
  units=45)로 실제 렌더링 확인 — cogsTotal/revenue=150%(다른 두 비율보다 압도적으로 커
  1순위 원인으로 정확히 지목됨), 반품률 11%(≥10% 임계값 통과)로 "매입원가가 매출의 150%예요.
  매입가를 낮추거나 판매가를 올리는 걸 검토해보세요." + "반품률이 11%로 높은 편이에요. 상품
  설명이나 품질을 점검해보세요." 두 문장이 정확히 출력됨을 Chrome MCP 스크린샷으로 확인.
  흑자 상품(흑자상품A/C)에는 이 패널 자체가 렌더링되지 않음(`items.length === 0` 이면
  `null` 반환)도 함께 확인.
- **범위 밖으로 남긴 것**: 진단 규칙 임계값(반품률 10% 등)은 초기값이라 베타 사용자 피드백에
  따라 조정될 수 있음 — 지금은 하드코딩, 설정화는 필요해지면 별도로 다룬다.
