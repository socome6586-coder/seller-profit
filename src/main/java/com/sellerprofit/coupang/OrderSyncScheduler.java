...
/**
 * 쿠팡 마켓 계정 전체를 주기적으로 순회하며 발주서를 수집한다.
 *
 * 한 계정의 실패가 나머지 계정 수집을 막지 않도록 계정 단위로 예외를 격리한다.
 */
@Component
public class OrderSyncScheduler {
    ...
    public OrderSyncScheduler(MarketAccountRepository marketAccountRepository,
                              OrderIngestionService ingestionService,
                              @Value("${coupang.default-lookback-days:7}") int defaultLookbackDays) {
        this.marketAccountRepository = marketAccountRepository;
        this.ingestionService = ingestionService;
        this.defaultLookbackDays = defaultLookbackDays;
    }

    /** 30분마다 전체 쿠팡 계정 발주서 수집. 초기 지연으로 부팅 직후 폭주를 피한다. */
    ...
    public void syncAllCoupangAccounts() {
        ...
        log.info("쿠팡 발주서 동기화 시작: 대상 계정={}개", accounts.size());
        ...
        for (MarketAccount account : accounts) {
            try {
                ...
                log.warn("계정 수집 실패: accountId={}, 원인={}", account.getId(), e.getMessage());
            }
        }
    }

    /** 마지막 동기화 시각이 있으면 그 날짜부터, 없으면 lookback 일 전부터 수집한다. */
    private LocalDate resolveFrom(MarketAccount account, LocalDate today) {
        if (account.getLastOrderSyncedAt() != null) {
            return account.getLastOrderSyncedAt().atZoneSameInstant(KST).toLocalDate();
        }
